...
title = "Гистограмма распределения гидротермического коэффициента Селянинова с 1967 по 2020 гг. " + r"за период %s - %s," % (begin_date.strftime("%d.%m"), end_date.strftime("%d.%m")) + "\n" + r"показатель ГТК за %s" % (inter_year)
...
# plt.title(title)
#plt.show()
title = "График изменчивости гидротермического коэффициента Селянинова с 1967 по 2020 гг.\n" + r"период %s - %s" % (begin_date.strftime("%d.%m"), end_date.strftime("%d.%m"))
plt.title(title, fontsize=12)
var1.plot()
plt.show()